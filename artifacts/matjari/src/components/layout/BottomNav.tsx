...
export default function BottomNav({ slug, itemCount, wishlistCount, onOpenCategories }: BottomNavProps) {
  ...
  return (
    <nav
      ...
      className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white/95 backdrop-blur-md border-t border-zinc-200"
          ...
          const content = (
            <>
              <span className="relative">
                ...
                {!!badge && badge > 0 && (
                  <span className="absolute -top-1.5 -left-2 min-w-[15px] h-[15px] px-1 bg-zinc-900 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                    {badge}
                  </span>
                )}
              </span>
              <span className="text-[10px] leading-none tracking-wide">{label}</span>
              {active && <span className="absolute top-0 inset-x-5 h-0.5 bg-zinc-900 rounded-full" />}
            </>
          );
