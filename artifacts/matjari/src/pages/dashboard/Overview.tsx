        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-amber-200 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg font-bold text-amber-900">تنبيه المخزون المنخفض</h3>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-200 text-amber-800">
                {stats.lowStockCount} منتج
              </span>
            </div>
            <Link href="/dashboard/products" className="text-sm text-primary hover:underline">إدارة المنتجات</Link>
          </div>
          <ul className="divide-y divide-amber-100">
            {stats.lowStockItems.slice(0, 5).map(item => (
              <li key={item.variantId} className="px-6 py-3 flex items-center justify-between">
                <div>
                  <span className="font-medium text-gray-900">{item.productName}</span>
                  <span className="text-sm text-gray-500"> — {item.variantLabel}</span>
                </div>
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${
                  ...
                  {item.stock === 0 ? 'نفد المخزون' : `متبقي ${item.stock}`}
                </span>
              </li>
            ))}
          </ul>
