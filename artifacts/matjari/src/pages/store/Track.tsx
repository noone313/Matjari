  ...
  return (
    <div className="max-w-xl mx-auto">
      <div className="text-center mb-10">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white mb-5">
          <PackageSearch className="w-8 h-8" />
        </div>
        <h1 className="font-serif font-bold text-4xl text-gray-900 dark:text-white">تتبع طلبك</h1>
      ...
      <form
        ...
        className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm p-6 sm:p-8 space-y-5"
      >
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">رقم الطلب</label>
          <Input
            type="text"
            inputMode="numeric"
            dir="ltr"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">رقم الهاتف</label>
          <Input
            type="tel"
            dir="ltr"
        ...
        {error && (
          <p className="text-sm text-red-600 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-lg px-3 py-2.5">
      ...
      {order && (
        <div className="mt-10 bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm p-6 sm:p-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-sm text-gray-500 dark:text-zinc-400">طلب رقم</p>
              <p className="text-2xl font-bold font-mono text-gray-900 dark:text-white">#{order.id}</p>
            </div>
          ...
          <p className="text-xs text-gray-400 dark:text-zinc-500 mb-6" dir="ltr">
              ...
              {ORDER_STEPS.map((step, idx) => {
                ...
                return (
                  <React.Fragment key={step}>
                    {idx > 0 && (
                      <div className={`flex-1 h-0.5 ${isReached ? 'bg-zinc-900 dark:bg-white' : 'bg-gray-200 dark:bg-zinc-700'}`} />
                    )}
                    <div className="flex flex-col items-center gap-1.5">
                      ...
                      <span className={`text-[11px] ${isReached ? 'text-gray-900 dark:text-white font-medium' : 'text-gray-400 dark:text-zinc-500'}`}>
                        {getStatusLabel(step)}
                      </span>
                    </div>
                  </React.Fragment>
          ...
          <div className="border-t border-gray-100 dark:border-zinc-800 pt-5">
            <p className="text-xs font-medium text-gray-500 dark:text-zinc-400 uppercase tracking-widest mb-3">المنتجات</p>
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 dark:border-zinc-800 space-y-1.5 text-sm">
            ...
            <div className="flex items-center justify-between pt-2 text-base font-bold text-gray-900 dark:text-white">
              <span>الإجمالي</span>
