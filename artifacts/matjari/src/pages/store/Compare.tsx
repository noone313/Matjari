...
function CompareColumn({ product, slug, onRemove }: { product: Product; slug: string; onRemove: (id: number) => void }) {
  ...
  return (
    <div className="bg-white flex flex-col" data-testid={`compare-col-${product.id}`}>
      {/* Image */}
      <div className="relative aspect-[3/4] bg-zinc-50 border-b border-zinc-100">
        <button
          onClick={() => onRemove(product.id)}
          aria-label={`إزالة ${product.name} من المقارنة`}
          className="absolute top-2 right-2 z-10 w-9 h-9 rounded-full bg-white/90 shadow-sm border border-zinc-200 flex items-center justify-center text-zinc-500 hover:text-white hover:bg-zinc-900 transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
        {product.imageUrls?.[0] ? (
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <Link href={`/store/${slug}/product/${product.id}`} className="font-serif text-lg font-bold text-zinc-900 leading-snug hover:text-primary transition-colors">
          {product.name}
        </Link>
        ...
        <div className="mt-5 border-t border-zinc-100">
          {fragrance && (product.noteTop || product.noteHeart || product.noteBase) ? (
            ...
            <div className="space-y-4 py-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <span className="text-gray-600 text-sm leading-relaxed">{product.skinType || '—'}</span>
                </div>
              </div>
            ...
            <p className="py-5 text-gray-500 text-sm">لا توجد تفاصيل إضافية</p>
          )}
        </div>

        {product.description && (
          <p className="mt-4 text-sm text-gray-500 leading-relaxed line-clamp-3">{product.description}</p>
        )}
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-serif text-2xl font-bold text-zinc-900">مقارنة المنتجات</h1>
          ...
          {items.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 text-xs tracking-widest uppercase border border-zinc-200 px-4 py-2 text-zinc-500 hover:border-zinc-900 hover:text-zinc-900 transition-colors"
            >
      ...
      {items.length === 0 ? (
        <div className="py-32 text-center">
          <Scale className="w-12 h-12 mx-auto text-zinc-200 mb-4" />
          <p className="text-sm tracking-widest uppercase text-zinc-300 mb-6">لا توجد منتجات للمقارنة</p>
          <Link
            href={`/store/${slug}`}
            className="inline-block text-xs tracking-widest uppercase bg-zinc-900 text-white px-6 py-3 hover:bg-zinc-800 transition-colors"
          >
            تصفح المنتجات
          </Link>
        </div>
