  ...
  if (wishlistProducts.length === 0) {
    return (
      <div className="py-32 text-center">
        ...
        <p className="text-sm tracking-widest uppercase text-zinc-300 mb-6">لا توجد منتجات في المفضلة بعد</p>
        <Link
          href={`/store/${slug}`}
          className="inline-flex items-center gap-2 text-xs font-bold tracking-widest uppercase px-6 py-3 bg-zinc-900 text-white hover:bg-zinc-800 transition-colors"
        >
          تصفح المنتجات
        </Link>
      </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-serif font-bold text-3xl text-gray-900 flex items-center gap-3">
          ...
          المفضلة
          <span className="text-sm font-normal text-zinc-400">({wishlistProducts.length})</span>
        </h1>
      </div>
          ...
          return (
            <div key={product.id} className="bg-white group">
              ...
              <button
                ...
                className="absolute -mt-20 mr-3 z-10 flex items-center gap-1.5 text-[10px] tracking-widest uppercase px-3 py-1.5 shadow-sm transition-colors bg-white text-zinc-900 hover:bg-[hsl(var(--primary))] hover:text-white"
              >
              ...
              <div className="p-4 pb-5">
                ...
                <Link href={`/store/${slug}/product/${product.id}`}>
                  <h3 className="text-sm font-medium text-zinc-900 leading-snug line-clamp-1 font-serif hover:text-[hsl(var(--primary))] transition-colors">
                    {product.name}
                  </h3>
                </Link>
                <div className="mt-2 flex items-center justify-between">
                  <span className="text-sm text-zinc-700">{formatPrice(minPrice)}</span>
                  <Link
